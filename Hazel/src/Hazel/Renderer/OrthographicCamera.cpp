...
namespace Hazel {
	...
	//last:
	//（Camera 的position，以及他的旋转的关系）//（也可以是lookat矩阵，3个vec3(pos,up,toward)//后面两个都是单位向量），这个lookat 的矩阵返回的就直接是viewprojection
	
	//next:expain the origin therop:
	//目的是把camera固定在原点，looking at -z;
	//inverse就是求矩阵的逆
	//  先Rotation-》translation-》inverse—》projection(逻辑顺序，从右往左)---viewProjection
	...
	void OrthographicCamera::RecalculateViewMatrix()
	{
		...
		//旋转会变形的根本原因是先旋转和移动和先移动后旋转的最终位置是不一样的，会变形
