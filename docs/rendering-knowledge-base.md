...
7. [HDR 等距柱状图天空上下颠倒：投影公式与文件行序不一致](#问题-7hdr-等距柱状图天空上下颠倒投影公式与文件行序不一致)
8. [通用调试方法](#通用调试方法)
9. [相关资产说明](#相关资产说明)
10. [遗留问题](#遗留问题)
...
## 问题 7：HDR 等距柱状图天空上下颠倒（投影公式与文件行序不一致）

**现象**：拖入 `.hdr`（如 birchwood_4k.hdr）后天空盒能显示，但方向不对（上下颠倒）。

**根因**：Radiance HDR 的分辨率行是 `-Y <height> +X <width>`，`-Y` 表示**文件第一行 = 图像顶部 = 天空（天顶）**；stb_image 不翻转时缓冲第 0 行 = 文件第一行，对应纹理 v=0。而投影公式 `v = asin(dir.y)/PI + 0.5` 把天顶（+Y）映射到 v=1 = 图像底部 = 地面 → 天空被渲染到脚下。

**修复（共两处）**：
1. `OpenGLTextureCube` 的等距采样改为 `v = 0.5 - asin(dir.y)/PI`（天顶 → v=0 → 文件顶部）。
2. **面顺序映射**：等距投影生成的面顺序是 `+X,-X,+Y,-Y,+Z,-Z`（faces[0..5]），而十字提取的顺序是 `[-X,+Z,+X,-Z] + 上/+Y、下/-Y`。两者上传到 `GL_TEXTURE_CUBE_MAP_*` 的索引必须分开，否则六个面会互相错位（+X 里装的是 +Y 的内容等），表现就是“天空盒六个面顺序不对”。

**排查方法**：
- 解析 `.hdr` 文件头，确认分辨率行是 `-Y`（从上到下）还是 `+Y`（从下到上）；
- 确认 stb_image 对 Radiance 文件按文件顺序输出、`stbi_set_flip_vertically_on_load(false)` 不翻转；
- 结合 OpenGL 立方体贴图规范的面方向约定逐项核对。

**相关**：TGA 十字路径（Debug.tga）不受影响——stb 的 TGA 加载器会把 bottom-up 文件归一成 top-down，且十字提取与 OpenGL 采样约定一致。
